...
void	dda_algo(t_ray *ray, t_map *map)
{
	while (ray->hit == 0)
	{
		...
		{
printf("sideX1: %f | sideY1: %f | mapX1: %d | mapY1: %d\n", ray->side_distX, ray->side_distY, ray->mapX, ray->mapY);
			ray->side_distX += ray->delta_distX;
			ray->mapX += ray->step_x;
			ray->side_hit = 0;
printf("sideX2: %f | sideY2: %f | mapX2: %d | mapY2: %d\n", ray->side_distX, ray->side_distY, ray->mapX, ray->mapY);
		}
		else
		{
...
void	raycaster(t_cub *cub)
{
	...
	while (x < SCREEN_WIDTH)
	{
		...
		ray.dir.x = cub->fov.dir.x + cub->fov.plane.x * cub->fov.cameraX;
		ray.dir.y = cub->fov.dir.y + cub->fov.plane.y * cub->fov.cameraX;
